...
\end{align}
we thus ensure stationary of the integration step.
Since we resample the momentum immediately after each integration step, and $K(r) = K(-r)$ we do not need to negate the momentum in practice.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}
        \Inputs{
            $U(x) = -\log{p(x)}$ (Potential energy function) \\ 
            $K$ (number of samples) \\ 
            $\epsilon>0$ (step size) \\
            $T$ (number of steps) \\
            $M$ (Mass matrix)},
        \Outputs{Samples $\{x^{(k)}|k\in 1,\dots,K\}$}
        \For{$k=1,\dots,K$}
        ...
        \IfThenElse{$i < T$}{$r\gets r - \epsilon \nabla U(x)$}{$r\gets r - \epsilon \nabla U(x)/2$}
        \EndFor
        \State Sample $u \sim \mathcal{U}(0, 1)$
        \If{$u \leq \exp[-H(x^{(k-1)}) + H(x)]$}
        \State $x^{(k)} \gets x$
        \Else
...
\end{align}
where $W_t$ is a $D$ dimensional Weiner process.
...
discretizing system in the \crefrange{eq:ds1}{eq:ds2}. 
...
\begin{algorithm}[htpb]
    ...
    \begin{algorithmic}
        \Inputs{$\tilde{U}(\theta) \approx U(\theta)$ (Noisy potential energy function, e.g. $-\log{p(\theta|\tilde\D)} - \log{p(\theta)}$) \\
        $K$ (number of samples) \\
        $\eta>0$ (learning rate) \\
        $\alpha$ (Momentum decay) \\
        $\hat{\beta}$ (Variance estimate) \\ 
        $T$ (number of steps) \\
        $M$ (Mass matrix)}
        \Outputs{Samples $\{\theta^{(k)}\mid k\in 1,\dots,K\}$}
        \For{$k=1,\dots, K$}
...
Another approach to sampling from the posterior distribution, $p(\theta | \D)$, is described by \autocite{blundell_weight_2015}. 
...
In \autocite{blundell_weight_2015} and this project, we will only consider Gaussian distributions for the variational distribution.
...
The specifics of the optimization algorithms are omitted.} \label{alg:vi}
    \begin{algorithmic}
        \Inputs{$p(y|x, \theta)$ (Model likelihood) \\ 
        $p(\theta)$ (Parameter prior) \\ 
        $q_\psi(\theta)$ (Variational posterior) \\ 
        $\D=(\bm{X}, \bm{Y})$ (Data)  \\
        $N_\text{opt}$ (Number of optimization steps) \\
        $K$ (Number of estimation samples) \\
        $M$ (Number of batches)
        }
        \Outputs{Optimized variational parameters $\psi=(\mu, \rho)$}
        \For{$i=1,\dots, N_\text{opt}$}
