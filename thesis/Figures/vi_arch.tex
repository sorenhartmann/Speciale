...
\begin{tikzpicture}[
    ...
    --
    ($(tmp1)!2.0!(tmp1 |- model)$) node[midway,anchor=center,label={left:sample $n$}] (sample-n) {}
    ...
    (get-obs.center) 
    arc (90:0:\radcirc) 
    arc (360:90:\radcirc) node[pos=0.333,anchor=north] (label) {repeat $n$ times}
    ;
    ...
    \draw [->] (obs) edge (model);
    \draw [->] (model) edge (loss);
    \draw [->] (sample-n.center) -- (model);
